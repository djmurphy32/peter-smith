...
describe('Portfolio.vue', () => {
  describe('GIVEN initial state', () => {
    ...
    it('THEN renders every image in the portfolio directory', () => {
      ...
      expect(images.at(0).props().src).toStrictEqual(`image1.jpg${queryString}`)
      expect(images.at(1).props().src).toStrictEqual(`image2.jpg${queryString}`)
      expect(images.at(2).props().src).toStrictEqual(`images/test/image3.jpg${queryString}`)
    })
